...
enum LinkstrMessagePayloadCodec {
  private static let payloadQueryKey = "p"
  private static let messageURLScheme = "https"
  private static let messageURLHost = "linkstr.app"
  private static let messageURLPath = "/messages/open"
  private static let appDeepLinkScheme = "linkstr"
  private static let appDeepLinkHost = "open"

  static func encode(_ payload: LinkstrDeepLinkPayload) -> String? {
  ...
  static func decode(_ token: String) -> LinkstrDeepLinkPayload? {
    var base64 =
      token
      .replacingOccurrences(of: "-", with: "+")
  ...
  static func makeMessageURL(payload: LinkstrDeepLinkPayload) -> URL? {
    guard let token = encode(payload) else {
      return nil
    }

    var components = URLComponents()
    components.scheme = messageURLScheme
    components.host = messageURLHost
    components.path = messageURLPath
  ...
  static func parsePayload(fromMessageURL url: URL) -> LinkstrDeepLinkPayload? {
    guard url.scheme?.lowercased() == messageURLScheme else {
      return nil
    }

    guard let host = url.host?.lowercased(),
      host == messageURLHost || host == "www.\(messageURLHost)"
    else {
      return nil
    }

    guard url.path == messageURLPath else {
      return nil
    }
