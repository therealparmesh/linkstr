...
final class AppSessionIngestTests: AppSessionTestCase {
  ...
  func testHistoricalRootRestoreDoesNotTriggerLocalNotificationButLiveRootDoes() throws {
    var notifiedEventIDs: [String] = []
    let (session, container) = try makeSession(
      onIncomingPostNotification: { eventID in
        notifiedEventIDs.append(eventID)
      }
    )
    _ = container
    try session.identityService.createNewIdentity()
    ...
    let creatorPubkey = try TestKeyMaterialFactory.makePubkeyHex()
    let peerPubkey = try TestKeyMaterialFactory.makePubkeyHex()
    let sessionID = "session-root-notification-filter"

    session.ingestForTesting(
      makeIncomingMessage(
        eventID: "session-create-root-notification-filter",
        senderPubkey: creatorPubkey,
        ...
        payload: LinkstrPayload(
          ...
          note: nil,
          timestamp: 1900,
          sessionName: "Root Notification Filter",
          memberPubkeys: [creatorPubkey, myPubkey, peerPubkey]
        )
      ))
    ...
    session.ingestForTesting(
      makeIncomingMessage(
        eventID: "root-historical-no-notify",
        senderPubkey: peerPubkey,
        createdAt: Date(timeIntervalSince1970: 1905),
        payload: LinkstrPayload(
          conversationID: sessionID,
          rootID: "root-historical-no-notify",
          kind: .root,
          url: "https://example.com/root-historical-no-notify",
          note: nil,
          timestamp: 1905
        ),
        source: .historical
    ...
    session.ingestForTesting(
      makeIncomingMessage(
        eventID: "root-live-notify",
        senderPubkey: peerPubkey,
        createdAt: Date(timeIntervalSince1970: 1910),
        payload: LinkstrPayload(
          conversationID: sessionID,
          rootID: "root-live-notify",
          kind: .root,
          url: "https://example.com/root-live-notify",
          note: nil,
          timestamp: 1910
        ),
        source: .live
    ...
    XCTAssertEqual(notifiedEventIDs, ["root-live-notify"])
  }

  func testHistoricalReactionRestoreDoesNotTriggerLocalNotificationButLiveReactionDoes()
    throws
  {
    var notifiedEventIDs: [String] = []
    let (session, container) = try makeSession(
      onIncomingReactionNotification: { eventID in
        notifiedEventIDs.append(eventID)
      }
    )
    _ = container
    ...
    let creatorPubkey = try TestKeyMaterialFactory.makePubkeyHex()
    let peerPubkey = try TestKeyMaterialFactory.makePubkeyHex()
    let sessionID = "session-reaction-notification-filter"
    let rootEventID = "reaction-notification-root"

    session.ingestForTesting(
      makeIncomingMessage(
        eventID: "session-create-reaction-notification-filter",
        senderPubkey: creatorPubkey,
        ...
        payload: LinkstrPayload(
          ...
          note: nil,
          timestamp: 2000,
          sessionName: "Reaction Notification Filter",
          memberPubkeys: [creatorPubkey, myPubkey, peerPubkey]
        )
      ))
    ...
    session.ingestForTesting(
      makeIncomingMessage(
        ...
        createdAt: Date(timeIntervalSince1970: 2005),
        payload: LinkstrPayload(
          conversationID: sessionID,
          rootID: rootEventID,
          kind: .root,
          url: "https://example.com/reaction-notification-root",
          note: nil,
          timestamp: 2005
        )
      ))
    ...
    session.ingestForTesting(
      makeIncomingMessage(
        eventID: "reaction-historical-no-notify",
        senderPubkey: peerPubkey,
        createdAt: Date(timeIntervalSince1970: 2010),
        payload: LinkstrPayload(
          conversationID: sessionID,
          rootID: rootEventID,
          kind: .reaction,
    ...
    session.ingestForTesting(
      makeIncomingMessage(
        eventID: "reaction-live-notify",
        senderPubkey: peerPubkey,
        createdAt: Date(timeIntervalSince1970: 2015),
        payload: LinkstrPayload(
          conversationID: sessionID,
          rootID: rootEventID,
          kind: .reaction,
    ...
    XCTAssertEqual(notifiedEventIDs, ["reaction-live-notify"])
  }
