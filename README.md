...
- Messaging
  ...
  - Notifications are local best-effort alerts triggered by incoming relay events (no APNs remote push yet).
  - Foreground behavior: notifications are still presented as banner/list/sound while the app is open.
- Deep links
  - Main app handles `linkstr://open?p=...` payloads and opens an isolated playback-first screen with `Done` to dismiss.
...
- `Log Out and Clear Local Data` removes this account's local contacts/messages, cached local video references, and this account's local-data encryption key.
