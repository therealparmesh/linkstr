...
struct LinkstrDangerButtonStyle: ButtonStyle {
  func makeBody(configuration: Configuration) -> some View {
    configuration.label
      .font(.custom(LinkstrTheme.bodyFont, size: 14))
      .foregroundStyle(Color.white)
      ...
      .background(
        RoundedRectangle(cornerRadius: 12, style: .continuous)
          .fill(Color.red.opacity(configuration.isPressed ? 0.72 : 0.85))
      )
  }
}

struct LinkstrWarningButtonStyle: ButtonStyle {
  func makeBody(configuration: Configuration) -> some View {
    configuration.label
      .font(.custom(LinkstrTheme.bodyFont, size: 14))
      .foregroundStyle(LinkstrTheme.bgBottom)
      .lineLimit(2)
      .multilineTextAlignment(.center)
      .padding(.horizontal, 14)
      .padding(.vertical, 10)
      ...
      .background(
        RoundedRectangle(cornerRadius: 12, style: .continuous)
          .fill(LinkstrTheme.neonAmber.opacity(configuration.isPressed ? 0.72 : 0.92))
      )
  }
}
